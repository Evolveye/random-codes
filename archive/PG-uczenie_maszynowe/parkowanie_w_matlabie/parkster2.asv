% SYMULACJA SAMOCHODU - sterowanie reczne lub rozmyte + pokaz regul
clear;
...
% Parametry zadania:
max_liczba_krokow = 420;                 % liczba krokow, po przekroczeniu ktorej jazda jest przerywana
...
%  ...............  tutaj wpisz wlasne stany poczatkowe
...
pokaz_regul = true;            % pokaz regul podczas wnioskowania
...
if strcmp(typ_ster,'rozmyte')
    ...
    if pokaz_regul                                                    % sledzenie wnioskowania rozmytego
        ruleview(fis);                                                % otwarcie okna regul
        xx = allchild(0);                                             % wszystkie okna programu
        for i=1:length(xx)                                            % przechwycenie wskaznika okna regul
            name=get(xx(i), 'name');
            if length(name)>11 & strcmp(name(1:12), 'Rule Viewer:');
                figReg=xx(i);                                         % to jest ten wskaznik
                break;
            end
        end
        ekran = get(0,'screensize');                                  % rozmiary ekranu
        %set(figN, 'Position',[ekran(3)/2,0,ekran(3)/2,ekran(4)/2]);
        set(figReg, 'HandleVis', 'on');                               % sam nie wiem po co to potrzebne
    end
...
ekran = get(0,'screensize');                                 % rozmiary ekranu
zmn = min(1,ekran(4)/(1.4*ekran(3)*prop));                   % jesli okno sie nie miesci na ekranie - trzeba zmniejszyc
zmn = zmn - pokaz_regul*zmn/2;                               % dodatkowe zmniejszenie by pokazac okno regul
set(f,'Position',[0 ekran(4)-zmn*1.4*ekran(3)*prop-40   zmn*ekran(3)*0.95  zmn*1.4*ekran(3)*prop*0.95]);     % ustawienie pozycji oraz rozmiarow okna rysunku
...
for st=1:liczba_stanow_pocz                      % po stanach poczatkowych
   ...
   set(f, 'SelectionType','open');                 % ustawienie klawisza myszy 
   ...
   while (~koniec)     % po krokach czasowych 
      ...
      if strcmp(typ_ster,'reczne')
          Po = get(f, 'Position');              % pozycja i rozmiary biezacego okna na ekranie
          ...
          button = get(f, 'SelectionType');     % typ klawisza: 'normal', 'extend' lub 'alt', ('open' - podwojne szybkie klikniecie)      
      ...
      elseif strcmp(typ_ster,'rozmyte')
          ...
          kat = wynik(1);
          if (abs(wynik(2)) < 0.15)             % zatrzymanie pojazdu jesli predkosc mniejsza od pewnej granicy
              V = 0;
          else
              V = Vmod*sign(wynik(2));
          end
          ...
          if pokaz_regul
              ruleview('#simulink', stan, figReg);   % pokazanie wnioskowania rozmytego dla danego stanu
          end
      elseif strcmp(typ_ster,'neuro')
          wy = sim(net,stan');
          kat = max_kat*wy(1);
...
delete(id_auto);delete(id_kola);          % likwidacja obiektow rysunkowych
close(f);                                 % zamykanie okien
if pokaz_regul
    close(figReg);
end
