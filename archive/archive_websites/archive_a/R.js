...
const R = {
 ...
 ,tags : {
   __constructor(ele=null ,root=false) {
      ...
      for(let tag of RTags) {
        ...
        if(!R.framework.tags.includes(Rtag)) {
 ...
 ,getFile(src ,scriptRemove=true ,cb) {
      ...
      let lang   = isSite ? 'www' : src.split(/\./g).splice(-1)[0].toUpperCase()
      let toDocHead
      let imported
        ...
        default:
          {
            ...
            try{ xhttp.send() } catch(err) { cb(null) }
      ...
      if(imported) imported.addEventListener('load' ,function(e) {
        if(cb) {
