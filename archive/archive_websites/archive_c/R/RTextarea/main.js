...
Paf.modules.textarea = {constructor : class {
 /** Constructor
  ...
  constructor(initializer) {
    ...
    //* DOM
      let RTextarea  =  document.createElement('div')
          RTextarea.className = `RTag RTextarea ${initializer.className}`
    ...
    let CSSeditor    = window.getComputedStyle(editor)

    this.config = {
      caretTime : 5
     ,disabled  : initializer.disabled
     ,fontSize  : Number(CSSeditor.fontSize.slice(0,-2))
     ,id        : Object.keys(R.tags.textarea).length
     ,text      : initializer.value
                    .split('\n')
                    .filter(item => /\s{0,}\|.*/.test(item) )
                    .map(item => item.replace(/\s{0,}\|/,'') )
    }

    let config  = this.config
    let id      = config.id
    this.ele.id = `RTextarea-${id}`
    ...
    //* Individual CSS
      let individualCSS = document.createElement('style')
          ...
          individualCSS.textContent = `
            ...
            ${!config.disabled ? `
            #RTextarea-${id} .editor *::-moz-selection {
              background: transparent;
            }
            #RTextarea-${id} .editor *::selection {
              background: transparent;
            }
            ` : ''}
            ...
            #RTextarea-${id} .rangeEntity {
              height: ${config.fontSize + 2}px;
            }

            #RTextarea-${id} .invisible {
              font-size: ${config.fontSize}px;
            }
          `
    ...
    if(config.text.length)
      for(let text of config.text)
         this.line_create({text})
    else this.line_create()

    setInterval(() => { this.caretGleaming() },100)
    let CSSlinesnums = window.getComputedStyle(linesnums)
